							...
							<section class="post">
								...
								<!-- Table -->
									<?php
										...
										if(empty($_POST['name']) || empty($_POST['memo']) || empty($_FILES['picture']['tmp_name'])){
										...
										if (filesize($_FILES['picture']['tmp_name']) > 3 * 1024 * 1024) {
												$response->error = "File Size Error (less than 3MB file only)";
												exit(json_encode($response));
										}


										$name = mysqli_real_escape_string($dbc, trim($_POST['name']));
										$id = mysqli_real_escape_string($dbc, trim($_SESSION['id']));
										...
										$query = "insert into review values(null, '$id', '$name', NOW(), '$image', '$memo')";
